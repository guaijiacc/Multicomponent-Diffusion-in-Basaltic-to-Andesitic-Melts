% calculate the derivates of r with respect to P(matrix of eigenvector) and beta. 
% J7 differs from J in that J7 doesn't return the derivatives
% of r with respect to the 7th terms of each eigenvectors. Therefore, J7 is
% only used to calculate the error while assuming that the errors of the 7th terms 
% of each eigenvectors is zero.

function J=calculate_J7_par(data_d,beta)
    ...
    [n,~]=size(J1);
    J=zeros(n,29*m+42+7*N_eig);
    for i=0:6
        J(:,29*m+6*i+1:29*m+6*i+6)=J1(:,29*m+7*i+1:29*m+7*i+6);
    end
    J(:,1:29*m) = J1(:,1:29*m);
    J(:,29*m+43:29*m+42+7*N_eig)=J1(:,29*m+50:29*m+49+7*N_eig);
end